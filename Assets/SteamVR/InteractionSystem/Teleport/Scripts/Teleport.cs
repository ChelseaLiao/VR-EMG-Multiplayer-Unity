...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		public Transform lockedReticleTransform;
		...
		public float minArcDistance = 3f;
		...
		public Hand TeleportHand => pointerHand;
		...
		private Hand oldPointerHand = null;
		private Hand newPointerHand = null;
		private Vector3 currentPointerEndPosition;
		...
		public Camera vrCamera, fallbackCamera;
		public Transform CurrentViewpoint => vrCamera == null || !vrCamera.isActiveAndEnabled ? fallbackCamera.transform : vrCamera.transform;
		//-------------------------------------------------
		public void UpdatePointer(float chargingStrength)
		{
			canTeleport = false;
			...
			pointerDir = CurrentViewpoint.transform.forward;
			
			pointerDir.y = 0;
			pointerDir = Vector3.Normalize(pointerDir);

			bool hitSomething = false;
			bool showPlayAreaPreview = false;
			Vector3 playerFeetOffset = player.trackingOriginTransform.position - player.feetPositionGuess;
			Vector3 pointerEnd;
			//arcDistance += chargingStrength;
			Vector3 arcVelocity = (pointerDir * minArcDistance) + (chargingStrength * pointerDir);

			PointableArea hitTeleportMarker = null;
			...
			{
				hitSomething = true;
				hitTeleportMarker = hitInfo.collider.GetComponentInParent<PointableArea>();
			}
			...
			if (hitTeleportMarker != null) //Hit a teleport marker
			{
				...
				if (hitTeleportMarker.locked)
				{
					...
					lockedReticleTransform.gameObject.SetActive( true );
				}
				else
				{
					canTeleport = true;
					...
					lockedReticleTransform.gameObject.SetActive( false );
				...
				// We are not using any player area markers
    ...
    //            }
			...
			currentPointerEndPosition = hitInfo.point;
			...
			lockedReticleTransform.position = pointedAtPosition;
		...
		{
			Debug.Log("Hide teleport pointer");
			if ( visible )
			{
		...
		{
			Debug.Log("Show teleport pointer");
			if ( !visible )
			{
				pointedAtTeleportMarker = null;
		...
		//-------------------------------------------------
		public bool TryTeleportPlayer()
		{
			if ( visible && !teleporting && canTeleport)
			{
				//if (pointedAtTeleportMarker != null && pointedAtTeleportMarker.locked == false)
				if (currentPointerEndPosition != null)
				{
				...
				{
					Debug.Log("Area locked or pointer points to nowhere");
					return false;
				}
			}
			else
		...
		private void TeleportPlayer()
		{
            ...
            // Measure distance to ground on first teleport
            if (currentHeight < 0f)
            {

				RaycastHit raycastHit;
				if (Physics.Raycast(player.trackingOriginTransform.position, Vector3.down, out raycastHit, 100, 1))
				{
					currentHeight = Vector3.Distance(raycastHit.point, player.trackingOriginTransform.position);
				}
            }

            Debug.Log("Pos before teleport: " + player.trackingOriginTransform.position);
			Debug.Log("Marker pos: " + teleportingToMarker);

			//teleportingToMarker.ShouldMovePlayer()
			if (true )
			{
                Vector3 playerFeetOffset = player.trackingOriginTransform.position - player.feetPositionGuess;
                //Vector3 newPlayerPosition = teleportPosition + playerFeetOffset + currentPointerEndPosition;				
                Vector3 newPlayerPosition = currentPointerEndPosition;
                newPlayerPosition.y += currentHeight;
				player.trackingOriginTransform.position = newPlayerPosition;

				Debug.Log("Pos after teleport: " + player.trackingOriginTransform.position);

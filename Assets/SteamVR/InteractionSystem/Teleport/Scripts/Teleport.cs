...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		{
			canTeleport = false;
			Vector3 pointerStart = player.trackingOriginTransform.position;
			Vector3 pointerDir = player.trackingOriginTransform.forward;
			if (pointerStartTransform != null)
			{ 
				pointerStart = pointerStartTransform.position;
				pointerDir = pointerStartTransform.forward;
			}
			...
			Vector3 arcVelocity = pointerDir * (chargingStrength * minArcDistance);
    ...
    //            }
			...
			else if (hitInfo.collider != null)//Hit neither
			{
				destinationReticleTransform.gameObject.SetActive( false );
